...
4.  **Numerical Representation:** Be vigilant regarding Endianness. Reading a 16-bit sensor value in the wrong byte order (Big-Endian vs Little-Endian) will result in values that are thousands of units off, causing immediate and violent instability.
...
*   **The "Limp Home" Mixer:** In `lab_0_6_mixers.py`, we explore how a 3-motor drone can still maintain level hover by re-calculating the Mixer Matrix in real-time. This "fault-tolerant" logic is what allows damaged drones to land safely rather than falling like a stone.
...
*   **Win Condition:** Successfully running `lab_0_6_mixers.py` with one motor disabled and seeing the other three compensate for the lost torque and thrust.
